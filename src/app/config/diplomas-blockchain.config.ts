import { IdentidadUNIR, IDENTITY_ROLES, IDENTITY_TYPE } from '../model/identidad-unir';
export enum PURPOSE_TYPES {MANAGEMENT = 1, ACTION, CLAIM, ENCRYPTION}
export enum KEY_TYPES {ECDSA = 1, RSA}
export enum CLAIM_TYPES {TITULO_ACADEMICO = 4}

export const RCP_URL_HTTP = 'http://localhost:8545';
export const RCP_URL_WS = 'ws://localhost:8545';

// Direcciónes de los diferentes, usuarios
export const addressAlumno = '0x5c4756bb912dea209b94587d4d761ace5d321054';
export const addressUniversidad = '0x951d59346352577920dbb5dca241fc5c346fe950';
export const addressUniversidadClaim = '0xFc83780Dd8bc6eAE4DEe1f12F7976251D2753DfD';
export const addressEmpresa = '0x7a55fdcb796ba184fda57530af3303b5553efc56';

export let identidades = new Map<string, IdentidadUNIR>();
identidades.set(addressAlumno, new IdentidadUNIR(IDENTITY_TYPE.CLAIM_HOLDER,
    IDENTITY_ROLES.ALUMNO,
    addressAlumno,
    '0xCDa08398d531B34fCDF6abFa40CEfd6fb399DA1A',
    null, null));

identidades.set(addressUniversidad, new IdentidadUNIR(IDENTITY_TYPE.CLAIM_HOLDER,
    IDENTITY_ROLES.UNIVERSIDAD,
    addressUniversidad,
    '0x64fAF8cAa1E5D5ded13AFab5cF2b986121269C5C',
    addressUniversidadClaim,
    null));

identidades.set(addressEmpresa, new IdentidadUNIR(IDENTITY_TYPE.CLAIM_VERIFIER,
    IDENTITY_ROLES.EMPRESA,
    addressEmpresa,
    '0xb1a0dEB41a2D4891fD5a107DB3214Dcc8764D8f7',
    null, null));